...
<!-- ```python
>>> def count_neighbors(grid: np.array) -> np.array:
... """Count the neighbors of each cell, treating neighbors 'outside' the matrix as 0 and returning an array of the same shape."""
... padded = np.pad(grid, 1) # add 0s around the grid
... windows = np.stride_tricks.sliding_window_view(
...     padded, window_shape = (3,3)
... )
    # add up each window and subtract the central cell
... return windows.sum(axis = (2,3)) - grid 
``` -->
<!-- 
Instead, we'll dig into the suite of functions available in `numpy` to find `np.stride_tricks.sliding_window_view`, which -- as you might expect -- creates a window of a size determined by a parameter and returns a higher-dimensional array containing each window. Let's define a function to create windows of size `(3,3)` over our grid of cells:

```python
>>> def get_neighbors(grid: np.array) -> np.array:
... """Get padded windows for the provided grid."""
... padded = np.pad(grid, 1)
... return np.stride_tricks.sliding_window_view(
...     padded, window_shape = (3,3)
... )
...
While Conway theorized an infinite grid, ours is but 4x4, so we use `np.pad` to add zeroes around the outside of the grid so that our windows will include the cells at the edges. As a sanity check, we can look back at the first array to see that the first window has the top-left element (0) in the center atop its two living neighbors.

---
> *Note:* The `sliding_window_view` function is convenient for this demonstration, but *not* a good choice for convolutions in most contexts. Per the `numpy` [docs](https://numpy.org/devdocs/reference/generated/numpy.lib.stride_tricks.sliding_window_view.html), specialized functions such as `scipy.signal.fftconvolve` and the filtering functions in the `scipy.ndimage` module are better lower-level choices. Otherwise, ML libraries such as `tensorflow` and `pytorch` offer built-in, optimized convolution layers for constructing neural network models.
---

Next, to count the neighbors of each cell, we can take the sum of each window -->
